	...
	: CANExtended::ICanDevice(deviceId), canex(ex), busy(false)
{
	...
	{
		semaphore = osSemaphoreCreate(osSemaphore(semCanDevice), 4);
		WorkThreadDef.reset(new osThreadDef_t { CanWorkThread, osPriorityNormal, 4, 0});
	}
}
